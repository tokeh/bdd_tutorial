...
##Trouble shooting
These examples are only tested in IntelliJ IDEA Community Edition.  return
Sometimes it's helpful to run the Gradle __clean__ task if the application behaves strangely after switching between branches.  return
...
In addition to the standard Cucumber reports more detailed reports are generated by a Cucumber reporting plugin.
They can be configured in the runner class in the __src/test/java/__ directory. The reports can be found in the
__target/cucumber__ directory.
